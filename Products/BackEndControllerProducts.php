...
class BackEndControllerProducts{
	...
	public static function insertProduct(){
		...
		if(isset($_POST['productName_new'])){
			$name = BackEndControllerProducts::test_input_PR($_POST['productName_new']);
		...
		if(isset($_POST['productDescription_new'])){
			$description = BackEndControllerProducts::test_input_PR($_POST['productDescription_new']);
		}else{
		...
		if($_POST['productPrice_new']){
			$price = BackEndControllerProducts::test_input_PR($_POST['productPrice_new']);
		...
		if($sgn && $_FILES["productPhoto_new"]['tmp_name'][0] != ""){			
			$msg = "";
			$targetFolder = "imagesProducts/".$product->getID()."_/";
			mkdir($targetFolder);
	...
	public static function editProduct(){
		...
		if(isset($_POST['IDProduct_edit'])){
			$id = BackEndControllerProducts::test_input_PR($_POST['IDProduct_edit']);
		...
		if(isset($_POST['productName_edit'])){
			$name = BackEndControllerProducts::test_input_PR($_POST['productName_edit']);
		...
		if(isset($_POST['productDescription_edit'])){
			$description = BackEndControllerProducts::test_input_PR($_POST['productDescription_edit']);
		}else{
		...
		if($_POST['productPrice_edit']){
			$price = BackEndControllerProducts::test_input_PR($_POST['productPrice_edit']);
			//provera da li je broj
			if(is_numeric($price)){
					//round funkcija mozda moze da se iskoristi
					$c = round($price,2);
		...
		if($equal && $_FILES["productPhoto_edit"]['tmp_name'][0] == ""){
			...
			if($_FILES["productPhoto_edit"]['tmp_name'][0] != ""){
				...
				$destinationFolder = "imagesProducts/".$product->getID()."_/";
				$msgOut = "";
...
if(isset($_POST['submit_newProduct'])){
	BackEndControllerProducts::insertProduct();
	include_once "BackEnd_Products.php";
}elseif(isset($_POST['submit_editProduct'])){
	BackEndControllerProducts::editProduct();
	include_once "BackEnd_Products.php";
}
